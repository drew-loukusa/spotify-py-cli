...
class TestDelete:
    def test_delete_no_force_cancled(self):      
        result = runner.invoke(app, ["delete", TEST_PL_NAME], \
            input="n\n")
        assert result.exit_code == 0
    ...
    def test_delete_playlist_DNE(self):
        ...
        assert f"Playlist '{TEST_PL_NAME}' could not be deleted as it appears to not exist!" in result.stdout
        
class TestSearch:
    def test_search_no_name_provided(self):
        result = runner.invoke(app, ["search"])
        assert result.exit_code == 0
        assert "No name provided, listing all playlists..." \
            in result.stdout
    ...
    def test_search_name_provided_and_playlist_DNE(self):
        result = runner.invoke(app, ["search", TEST_PL_NAME])
        assert result.exit_code == 1
        assert f"Could not find {TEST_PL_NAME} in user's playlists." \
            in result.stdout